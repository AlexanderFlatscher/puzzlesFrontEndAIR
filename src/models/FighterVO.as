	...
	public class FighterVO extends EventDispatcher
	{
		...
		public function parseHandCards(xml:XML):void{
			for each(var card:XML in xml.Hand.Card){
				handCards.addItem(new CardVO(card.id, card.type, card.targetType, card.name, card.image, card.description, card.talent));
			}
		}
		
		public function parseDeckCards(xml:XML):void{
			for each(var card:XML in xml.Deck.Card){
				deckCards.addItem(new CardVO(card.id, card.type, card.targetType, card.name, card.image, card.description, card.talent));
			}
		}
		...
		public function parseUnitCards(xml:XML):void{
			...
			for(var i:int = 0; i < length; i++){
				...
				unitCards[i].addItem(new CardVO(card.id, card.type, card.targetType, card.name, card.image, card.description, card.talent));
			}
		}
		...
		public function parseSpellCards(xml:XML):void{
			for each(var card:XML in xml.Spells.Card){
				if(card.type == "Upgrade"){
					...
					spellCards.addItem(new CardVO(card.id, card.type, card.targetType, card.name, card.image, card.description, card.talent));
			}
		}
		
		public function parseCemeteryCards(xml:XML):void{
			for each(var card:XML in xml.Cemetery.Card){
				cemetryCards.addItem(new CardVO(card.id, card.type, card.targetType, card.name, card.image, card.description, card.talent));
			}
		}
	}
}